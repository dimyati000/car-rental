function printFormSewa(idSewa) {
  var link = base_url + "/DaftarSewa/cetak_sewa_penumpang?idSewa=" + idSewa;
...
function printReport() {
  ...
  var link = base_url + "/Laporan/cetak_laporan_penumpang?jenisMobil=" + jenisMobil + "&tanggal_awal=" + tgl_awal + "&tanggal_akhir=" + tgl_akhir;
  window.open(link, '_blank', 'width=1024, height=768')
}
...
function showLaporan() {
  ...
  var tgl_awal = $('#tgl_awal').val();
  var tgl_akhir = $('#tgl_akhir').val();
  var link = base_url + "/DaftarSewa/penumpang?tanggal_awal=" + tgl_awal + "&tanggal_akhir=" + tgl_akhir + "&jenisMobil=" + jenisMobil;
